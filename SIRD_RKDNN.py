"""
@author: xi'an Li
Benchmark Code of SIRD model using Runge-Kutta iteration and DNN
2022-06-18
"""
import os
import sys
...
def RK2S(t_day=1, beta=0.1, S_i=1.0, I_i=1.0):
    K1 = -beta * (S_i * I_i) / (S_i + I_i)
    K2 = -beta * (S_i + 0.5*K1) * (I_i + 0.5*K1) / (S_i + I_i + K1)
    K3 = -beta * (S_i + 0.5 * K2) * (I_i + 0.5 * K2) / (S_i + I_i + K2)
    K4 = -beta * (S_i + K3) * (I_i + K3) / (S_i + I_i + K3)
    print('lllll')
